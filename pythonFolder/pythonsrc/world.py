...
class world:
    def __init__(self, soft_robots: SoftRobots, forces: ForceContainer, sim_params: SimParams) -> None:
        ...
        ("UPDATE SYSTEM FOR NEXT TIME STEP")
    ...
    def print_sim_data(self):
        ...
        if cf and ff:
            if cf.get_num_collisions() > 0:
                ...
                (f"time: {self.curr_time:.4f} | iters: {self.stepper.iter} | con: 0 | "
            ...
            if cf.get_num_collisions() > 0:
                ...
                (f"time: {self.curr_time:.4f} | iters: {self.stepper.iter} | con: 0 | min_dist: N/A")
        elif ff:
            (f"time: {self.curr_time:.4f} | iters: {self.stepper.iter} | "
                  f"floor_con: {ff.num_contacts} | f_min_dist: {ff.min_dist:.6f}")
        else:
